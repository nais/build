...
use thiserror::Error;
use crate::DockerTagGenerateError::{FailedExecute, ParseGitShortSha};
...
pub enum Error {
    ...
    DockerTag(#[from] DockerTagGenerateError),
...
#[derive(Debug, Error)]
pub enum DockerTagGenerateError {
...
fn docker_tag(filesystem_path: &str) -> Result<String, DockerTagGenerateError> {
...
fn main() -> Result<(), Error> {
    ...
    let docker_image_name = cfg.release
        .docker_name_builder(config::docker::image_name::Config {
            registry: cfg.release.value().registry,
            tag: docker_tag(&args.source_directory)?,
            // from nais.yml?
